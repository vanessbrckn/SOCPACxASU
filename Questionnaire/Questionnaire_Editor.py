...
def initalize_page_UI(page_list):
    ...
    move_left_btn.clicked.connect(lambda : move_page_UI(layout, page_list.current_page_index-1, page_list))

    move_right_btn = QPushButton('move page order >')
    move_right_btn.clicked.connect(lambda : move_page_UI(layout, page_list.current_page_index+1, page_list))

    add_page_button = QPushButton('add page')
    add_page_button.clicked.connect(lambda: insert_page_UI(layout, page_list))

    prev_page_btn = QPushButton('previous page')
    prev_page_btn.clicked.connect(lambda: prev_page_UI(layout, page_list))
    prev_page_btn.setMinimumWidth(100)

    next_page_btn = QPushButton('next page')
    next_page_btn.clicked.connect(lambda: next_page_UI(layout, page_list))
    next_page_btn.setMinimumWidth(100)

    remove_page_btn = QPushButton('remove_page')
    remove_page_btn.clicked.connect(lambda: remove_page_UI(layout, page_list))

    header.addWidget(move_left_btn)
    header.addWidget(add_page_button)
    if (page_list.current_page_index != 0):
        header.addWidget(prev_page_btn)
    else:
        space = QWidget()
        space.setMinimumWidth(prev_page_btn.minimumWidth())
        header.addWidget(space)

    if (page_list.current_page_index != page_list.len()-1):
        header.addWidget(next_page_btn)
    else:
        space = QWidget()
        space.setMinimumWidth(next_page_btn.minimumWidth())
        header.addWidget(space)
...
def update_page(layout, page_list):
    ...
    layout.addLayout(initalize_page_UI(page_list))
...
def edit_to_skip_list(selected_list, option):
    ...
    for label in selected_list:
        option.skip_pages.add_obj(page_list.obj_at(int(label.text())).get_id())

def select_pages(select_these, skip_pages):
    for page_id in skip_pages.list:
        index = page_list.get_index_from_id(page_id)
        if index != -1:
            select_these.item(index).setSelected(bool(True))
